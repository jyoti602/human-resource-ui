...
export default function EmployeeLayout() {
  ...
  const pageMeta = getEmployeePageMeta(location.pathname);
  const userName = user?.full_name || user?.name || user?.username || "Employee";
  const userInitial = userName.charAt(0).toUpperCase();
  const leaveFormOpen = shouldOpenLeaveForm(location.search);
  ...
  return (
    <div className="flex min-h-screen bg-[radial-gradient(circle_at_top,_rgba(16,185,129,0.08),transparent_30%),linear-gradient(180deg,#f8fafc_0%,#ecfeff_100%)]">
          ...
          <div className="flex items-center justify-between gap-3 px-5 py-4 sm:hidden">
            ...
            <div className="relative">
              <button
                ...
                className="flex items-center gap-2 rounded-2xl border border-slate-200 bg-white px-3 py-2 text-left shadow-sm transition hover:border-slate-300 hover:bg-slate-50"
              >
          ...
          <div className="hidden flex-col gap-4 px-5 py-4 sm:flex sm:h-14 sm:flex-row sm:items-center sm:justify-between sm:px-6 sm:py-0 lg:px-8">
            ...
            <div className="flex items-center justify-end gap-3 self-end sm:self-auto">
              <div className="relative">
                <button
                  onClick={toggleProfileMenu}
                  className="hidden items-center gap-3 rounded-2xl border border-slate-200 bg-white px-3 py-2 text-left shadow-sm transition hover:border-slate-300 hover:bg-slate-50 sm:flex"
                >
                  <div className="flex h-8 w-8 items-center justify-center rounded-full bg-slate-900 text-sm font-semibold text-white">
                    {userInitial}
                  </div>
...
function getEmployeePageMeta(pathname) {
  const routeMap = {
    "/employee/dashboard": {
      title: "Employee Workspace",
      subtitle: "Access your HR tools and profile",
    },
    "/employee/profile": {
      title: "My Profile",
      subtitle: "Maintain your personal details and account information.",
    },
    "/employee/attendance": {
      title: "My Attendance",
      subtitle: "Track daily check-ins, check-outs, and attendance history.",
    },
    "/employee/applyleave": {
      title: "Apply for Leave",
      subtitle: "Submit leave requests and review your leave balance.",
    },
    "/employee/salary": {
      title: "My Salary",
      subtitle: "View your monthly payslip details and salary breakdown.",
    },
    "/employee/cases": {
      title: "My Cases & Requests",
      subtitle: "View and manage your leave requests, attendance issues, and other cases.",
    },
  };
...
function shouldOpenLeaveForm(search) {
  const params = new URLSearchParams(search);
  return params.get("openLeave") === "1";
}
